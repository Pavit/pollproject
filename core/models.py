...
class CustomUser(models.Model):
	...
	def check_friends(self):
		...
		compare_list = []
		user_fb_ids = CustomUser.objects.values_list('fb_id', flat=True).order_by('fb_id')
		friend_fb_ids = []
		for friend in frienddict:
			friend_fb_ids.append(friend["id"])
			matches = CustomUser.objects.filter(fb_id__in=friend_fb_ids)
		try:
		...
		#matched_friend = CustomUser.objects.get(fb_id = match)
